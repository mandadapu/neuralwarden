  ...
  return (
    ...
    >
      <div className="mt-6 space-y-3">
        {TACTICS.map((tactic) => (
          <div key={tactic.id} className="bg-[#1c2128] rounded-xl border border-[#30363d] p-5">
            ...
            <div className="space-y-1.5">
              {tactic.techniques.map((t) => (
                <div key={t} className="text-sm text-[#c9d1d9] flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#30363d] flex-shrink-0" />
                  <span className="font-mono text-xs text-[#8b949e]">{t.split(" — ")[0]}</span>
                  <span>{t.split(" — ")[1]}</span>
                </div>
              ))}
            </div>
