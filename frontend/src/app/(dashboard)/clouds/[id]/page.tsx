...
const STATUS_STYLES: Record<string, string> = {
  ...
  ignored: "border-[#1a3020] text-[#5a7068] bg-[#0a1a14]",
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <select
          ...
          className="px-3 py-2 border border-[#122a1e] rounded-lg text-sm bg-[#0a1a14] text-white focus:outline-none focus:ring-2 focus:ring-primary/20"
        >
          <option value="all">All types</option>
          <option value="config">Config issues</option>
          <option value="log">Log issues</option>
        </select>
        <button className="p-2 border border-[#122a1e] rounded-lg text-[#5a7068] hover:bg-[#0a1a14] transition-colors">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" />
          </svg>
        </button>
        <select className="px-3 py-2 border border-[#122a1e] rounded-lg text-sm bg-[#0a1a14] text-[#8a9a90]">
      ...
      {!loading && !error && issues.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-[#5a7068]">Run a scan to check for security issues.</p>
        </div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#0a1a14] border-b border-[#122a1e]">
                <th className="text-left text-xs font-semibold text-[#5a7068] uppercase tracking-wider px-5 py-3 w-10">Type</th>
                ...
                <th className="text-left text-xs font-semibold text-[#5a7068] uppercase tracking-wider px-5 py-3">Fix</th>
                <th className="text-left text-xs font-semibold text-[#5a7068] uppercase tracking-wider px-5 py-3">Status</th>
              </tr>
            </thead>
                ...
                <tr key={issue.id} className="hover:bg-[#0a1a14] transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    {issue.remediation_script ? (
                      ...
                      <span className="text-xs text-[#3a5548]">—</span>
                    )}
                  </td>
                  <td className="px-5 py-3.5 relative" ref={statusDropdownId === issue.id ? dropdownRef : undefined}>
                    ...
                    {statusDropdownId === issue.id && (
                      <div className="absolute right-0 top-full mt-1 w-36 bg-[#081510] border border-[#122a1e] rounded-lg shadow-lg z-20 py-1">
                        {ALL_STATUSES.map((s) => (
                          <button
                            key={s}
                            onClick={() => changeStatus(issue.id, s)}
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-[#0a1a14] transition-colors ${
                              issue.status === s ? "font-semibold text-primary" : "text-[#c0d0c8]"
                            }`}
                          >
                            {STATUS_LABELS[s]}
                          </button>
