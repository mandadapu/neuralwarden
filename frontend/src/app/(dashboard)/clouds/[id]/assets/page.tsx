    ...
    <div>
      ...
      {!loading && !error && assets.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-1">No assets discovered</h3>
          <p className="text-sm text-gray-500">Run a scan to discover cloud resources.</p>
        </div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3">Metadata</th>
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3">Discovered</th>
              </tr>
            </thead>
              ...
              {filtered.map((asset) => (
                <tr key={asset.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">{asset.asset_type.replace(/_/g, " ")}</div>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-gray-600">{asset.region || "—"}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-xs text-gray-500 font-mono">{truncateMetadata(asset.metadata_json)}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-gray-500">{relativeTime(asset.discovered_at)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
