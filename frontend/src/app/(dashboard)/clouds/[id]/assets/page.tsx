    ...
    <div>
      ...
      {!loading && !error && assets.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-[#5a7068]">Run a scan to discover cloud resources.</p>
        </div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#0a1a14] border-b border-[#122a1e]">
                ...
                <th className="text-left text-xs font-semibold text-[#5a7068] uppercase tracking-wider px-5 py-3">Region</th>
                <th className="text-left text-xs font-semibold text-[#5a7068] uppercase tracking-wider px-5 py-3">Metadata</th>
                <th className="text-left text-xs font-semibold text-[#5a7068] uppercase tracking-wider px-5 py-3">Discovered</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#0e1e16]">
              {filtered.map((asset) => (
                <tr key={asset.id} className="hover:bg-[#0a1a14] transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    ...
                    <div className="text-xs text-[#3a5548] mt-0.5">{asset.asset_type.replace(/_/g, " ")}</div>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-[#8a9a90]">{asset.region || "—"}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-xs text-[#5a7068] font-mono">{truncateMetadata(asset.metadata_json)}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-[#5a7068]">{relativeTime(asset.discovered_at)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
