  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <button className="inline-flex items-center gap-2 px-4 py-2 border border-gray-200 text-sm font-medium text-gray-600 rounded-lg hover:bg-gray-50 transition-colors">
          Disconnect VMs
        </button>
      </div>
      ...
      {!loading && !error && vmRows.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-1">No virtual machines found</h3>
          <p className="text-sm text-gray-500">Run a scan to discover compute instances.</p>
        </div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3">Open Issues</th>
                ...
                <tr key={row.asset.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-medium text-gray-900">{row.asset.name}</div>
                        <div className="text-xs text-gray-400">{row.zone}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={`text-sm font-medium ${row.openIssues > 0 ? "text-red-600" : "text-gray-500"}`}>
                      {row.openIssues}
                    </span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-gray-500">{row.ignoredIssues}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    {row.highestSeverity ? (
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-gray-600 capitalize">{row.purpose}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-gray-500">{relativeTime(row.lastScan)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
