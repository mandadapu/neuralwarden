...
export default function ConnectCloudPage() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left panel — branding */}
      <div className="w-80 min-w-80 bg-[#040a07] text-white flex flex-col justify-between p-8">
        <div>
          <Link href="/clouds" className="flex items-center gap-2 mb-10 text-[#3a5548] hover:text-white text-sm transition-colors">
          ...
          <p className="text-sm text-[#3a5548] leading-relaxed">
          ...
          <div className="mt-10 space-y-4">
            {[
              ...
              <div key={n} className="flex items-center gap-3">
                ...
                <span className={`text-sm ${step >= n ? "text-white" : "text-[#5a7068]"}`}>{label}</span>
              </div>
            ))}
          </div>
        </div>
        <p className="text-xs text-[#8a9a90]">Your credentials are encrypted and stored securely.</p>
      </div>

      {/* Right panel — content */}
      <div className="flex-1 bg-[#081510] flex items-start justify-center p-10 overflow-y-auto">
        <div className="w-full max-w-lg">
          {/* Step 1: Choose Provider */}
          {step === 1 && (
            <div>
              <h3 className="text-xl font-bold text-white mb-1">Choose your cloud provider</h3>
              <p className="text-sm text-[#5a7068] mb-8">Select the cloud platform you want to connect.</p>
              <div className="space-y-3">
                <button
                  onClick={() => selectProvider("gcp")}
                  className="w-full flex items-center gap-4 p-5 border-2 border-[#122a1e] rounded-xl hover:border-primary hover:bg-primary/5 transition-all text-left cursor-pointer"
                >
                  ...
                  <div>
                    ...
                    <div className="text-sm text-[#5a7068]">Connect your GCP project for security scanning</div>
                  </div>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3a5548" strokeWidth="2" className="ml-auto">
                    <path d="M9 18l6-6-6-6" />
                  </svg>
                </button>

                <div className="w-full flex items-center gap-4 p-5 border-2 border-[#0e1e16] rounded-xl bg-[#0a1a14] opacity-60 cursor-not-allowed">
                  ...
                  <div>
                    <div className="font-semibold text-[#5a7068]">Amazon Web Services</div>
                    <div className="text-sm text-[#3a5548]">Connect your AWS account</div>
                  </div>
                  <span className="ml-auto px-2.5 py-1 bg-[#122a1e] text-[#5a7068] text-xs font-semibold rounded-full">Coming Soon</span>
                </div>
                ...
                <div className="w-full flex items-center gap-4 p-5 border-2 border-[#0e1e16] rounded-xl bg-[#0a1a14] opacity-60 cursor-not-allowed">
                  ...
                  <div>
                    <div className="font-semibold text-[#5a7068]">Microsoft Azure</div>
                    <div className="text-sm text-[#3a5548]">Connect your Azure subscription</div>
                  </div>
                  <span className="ml-auto px-2.5 py-1 bg-[#122a1e] text-[#5a7068] text-xs font-semibold rounded-full">Coming Soon</span>
                </div>
              </div>
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-sm text-[#5a7068] mb-8">Provide your GCP credentials for NeuralWarden to scan your infrastructure.</p>

              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-[#c0d0c8] mb-1.5">GCP Project ID</label>
                  <input
                    type="text"
                    value={data.projectId}
                    onChange={(e) => setData((d) => ({ ...d, projectId: e.target.value }))}
                    placeholder="my-project-123"
                    className="w-full px-4 py-2.5 border border-[#122a1e] rounded-lg text-sm bg-[#0a1a14] text-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-[#c0d0c8] mb-1.5">Service Account Key (JSON)</label>
                  <div
                    onDrop={handleFileDrop}
                    onDragOver={(e) => e.preventDefault()}
                    className="border-2 border-dashed border-[#122a1e] rounded-lg p-6 text-center hover:border-primary/40 transition-colors"
                  >
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#3a5548" strokeWidth="1.5" className="mx-auto mb-2">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12" />
                    </svg>
                    <p className="text-sm text-[#5a7068] mb-2">Drag and drop your JSON key file here</p>
                    <label className="inline-flex items-center gap-1 px-3 py-1.5 bg-[#0c1e18] text-[#8a9a90] text-sm font-medium rounded-lg cursor-pointer hover:bg-[#122a1e] transition-colors">
                      Browse Files
                      <input type="file" accept=".json" onChange={handleFileSelect} className="hidden" />
                    </label>
                  </div>
                  <div className="my-3 flex items-center gap-3">
                    <div className="flex-1 h-px bg-[#122a1e]" />
                    <span className="text-xs text-[#3a5548]">or paste JSON below</span>
                    <div className="flex-1 h-px bg-[#122a1e]" />
                  </div>
                  <textarea
                    ...
                    rows={6}
                    className="w-full px-4 py-2.5 border border-[#122a1e] rounded-lg text-sm font-mono bg-[#0a1a14] text-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary resize-none"
                  />
                </div>
              ...
              <div className="flex justify-between mt-8">
                <button
                  onClick={handleBack}
                  className="px-5 py-2.5 text-sm font-medium text-[#8a9a90] border border-[#122a1e] rounded-lg hover:bg-[#0a1a14] transition-colors"
                >
                  Back
                </button>
                <button
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-sm text-[#5a7068] mb-8">Name your cloud and select which services to scan.</p>
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 border border-[#122a1e] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-[#0a1a14] text-white"
                  >
                    {PURPOSES.map((p) => (
                ...
                <div>
                  <label className="block text-sm font-medium text-[#c0d0c8] mb-3">Services to scan</label>
                  <div className="space-y-2.5">
                    {SERVICES.map((svc) => (
                      <label
                        key={svc.id}
                        className="flex items-center gap-3 p-3 border border-[#122a1e] rounded-lg hover:bg-[#0a1a14] cursor-pointer transition-colors"
                      >
                        <input
                          type="checkbox"
                          checked={data.services.includes(svc.id)}
                          onChange={() => toggleService(svc.id)}
                          className="w-4 h-4 text-primary border-[#1a3020] rounded focus:ring-primary"
                        />
          ...
          {step === 4 && (
            <div>
              ...
              <p className="text-sm text-[#5a7068] mb-8">Review your configuration and connect your cloud.</p>

              <div className="bg-[#0a1a14] rounded-xl p-5 space-y-3 mb-6">
                <div className="flex justify-between">
                  <span className="text-sm text-[#5a7068]">Provider</span>
                  <span className="text-sm font-medium text-white uppercase">{data.provider}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-[#5a7068]">Project ID</span>
                  <code className="text-sm font-mono text-white">{data.projectId}</code>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-[#5a7068]">Name</span>
                  <span className="text-sm font-medium text-white">{data.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-[#5a7068]">Purpose</span>
                  <span className="text-sm font-medium text-white capitalize">{data.purpose}</span>
                </div>
                <div className="flex justify-between items-start">
                  <span className="text-sm text-[#5a7068]">Services</span>
                ...
                <div className="flex justify-between">
                  <span className="text-sm text-[#5a7068]">Credentials</span>
